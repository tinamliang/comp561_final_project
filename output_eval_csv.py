...
def accuracy_for_read(input_file, output_file):
    query = None
    ...
    accuracy = match_found / len(overlapping_pair) if overlapping_pair else 0
    ret_value.append({'query': query, 'accuracy': f'{(accuracy*100):.2f}%'})
    print(f'Accuracy for read {query}: {(accuracy*100):.2f}%')
...
def process_read(header, sequence, ct):
   ...
   # print(f'Creating temporary FASTA file for read: {ct}...')
    ...
    accuracy = accuracy_for_read(input_file, 'overlaps_ground_truth.csv')
    all_accuracy_results.extend(accuracy)
    df_accuracy = pd.DataFrame(all_accuracy_results)
    df_accuracy.to_csv('eval_naive_output.csv', index=False)

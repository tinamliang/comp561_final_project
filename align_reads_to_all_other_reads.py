...
def process_read(header, sequence, ct):
  ...
  #  print(f'removing temporary file: {output_file}')
    ...
    read_accuracy_data = calculate_accuracy_for_read(input_file, 'overlaps.csv')
...
def accuracy_table():
    if all_accuracy_results:
        ...
        print(f"Successfully saved all accuracy results ({len(df_accuracy)} rows) to accuracy_table.csv")
